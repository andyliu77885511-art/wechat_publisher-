...
/* ====================================================================
   公众号定位选择器 v4（openclaw.ai 风格：深色毛玻璃卡片 + 发光选中态 + staggered）
   适配 Streamlit radio 单 p 标签 + <br> 换行结构
   ==================================================================== */
...
/* stagger 延迟入场 — 第1/4个卡片延迟0s，第2/5个延迟0.05s，第3/6个延迟0.1s */
div[data-testid="stRadio"] > div > label:nth-child(1),
div[data-testid="stRadio"] > div > label:nth-child(4) { animation-delay: 0s !important; }
div[data-testid="stRadio"] > div > label:nth-child(2),
div[data-testid="stRadio"] > div > label:nth-child(5) { animation-delay: 0.05s !important; }
div[data-testid="stRadio"] > div > label:nth-child(3),
div[data-testid="stRadio"] > div > label:nth-child(6) { animation-delay: 0.1s !important; }
...
/* 模拟三行结构：用 ::before 做 emoji 行（第一行），p::first-line 做第二行（名称），剩余做第三行 */
/* 但我们不修改 option 数据，直接让 p 内的 br 自然换行 */

/* 用 ::first-line + 额外规则精确控制 emoji 大小 */
div[data-testid="stRadio"] > div > label > div:last-child p {
...
/* 通过 p 内文本的字符特征控制：emoji 字符用 unicode-range 不支持，换用 ::before */
/* 这里统一让 p 里的 emoji 变大：使用相邻 sibling 不好使，用 CSS */
/* 更好的方式：radio option 的数据结构已经包含 emoji，我们通过样式控制整体效果 */

/* 简化为：直接让 p 内的文字整体居中对齐，font-size 统一，line-height 拉开 */
div[data-testid="stRadio"] > div > label > div:last-child p {
...
/* 第一行 emoji 大字号 */
div[data-testid="stRadio"] > div > label > div:last-child p::first-line {
...
div[data-testid="stRadio"] > div > label:hover {
    ...
    box-shadow: 0 10px 32px rgba(255,140,0,0.12), 0 0 0 1px rgba(255,140,0,0.06), inset 0 1px 0 rgba(255,200,80,0.06) !important;
}
div[data-testid="stRadio"] > div > label:hover > div:last-child p {
    color: rgba(255,210,120,0.85) !important;
}

/* 选中态：发光边框 + 双层光晕 + 橙色渐变背景 */
div[data-testid="stRadio"] > div > label:has(input:checked) {
    background: linear-gradient(145deg, rgba(255,140,0,0.18), rgba(255,165,0,0.08), rgba(255,140,0,0.04)) !important;
    ...
    box-shadow:
        ...
        inset 0 1px 0 rgba(255,210,80,0.1) !important;
    animation: borderPulse 3.2s ease-in-out infinite, cardIn 0.4s ease both !important;
    transform: translateY(-1px) !important;
}
div[data-testid="stRadio"] > div > label:has(input:checked) > div:last-child p {
    color: #FFCC80 !important;
    text-shadow: 0 0 20px rgba(255,140,0,0.12) !important;
}

/* 隐藏 radio 原始圆点（保留 input 功能性）*/
...
/* ===== 写作风格 Radio — 独立 key 隔离，不受定位选择器样式干扰 ===== */
/* 写作风格区域用 key="style_radio"，其 data-testid 不同，保持原 pill 风格 */
...
# ── 公众号定位选择器 v3（st.radio 改造为 2×3 卡片网格）─────────────────────────
#    CSS :has(input:checked) 精准控制选中态，不依赖 wrapper div 技巧

# 定位选项：label 用 markdown 多行文字（emoji + 名称 + 说明）
_POSITIONS = [
    ("📈", "财经",    "宏观/市场/投资深度分析"),
    ("💡", "科技",    "前沿技术与产品趋势"),
    ("🌿", "生活方式", "生活美学与消费决策"),
...
# 三行格式：emoji / 名称 / 描述，CSS 通过 ::first-line 等控制各层级
_POS_OPTIONS = [p[0] + chr(10) + p[1] + chr(10) + p[2] for p in _POSITIONS]

# 当前选中的定位名称 → 找到对应 option index
...
# 用 st.radio 渲染；CSS 已把它改造成 2×3 卡片网格
...
# 解析选中的名称（option 格式："{emoji} {name}\n{desc}"）
# 三行格式：[emoji, 名称, 描述] — 取 index 1 为名称
_selected_name = _selected_option.split(chr(10))[1].strip()
