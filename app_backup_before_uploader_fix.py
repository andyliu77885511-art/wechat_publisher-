...
.upload-progress-label-left {
    color: #FFA500;
    font-weight: 600;
}
.upload-progress-label-right {
...
if st.session_state.step == "upload":
    ...
    if uploaded:
        ...
        if file_size_mb > config.MAX_FILE_SIZE_MB:
            ...
            # ── 上传就绪卡片：进度条 + 预估处理时间 ──
            _sz = file_size_mb
            # 预估：文件保存 2MB/s + 音视频转录按时长估算（粗算 1min 音频 ≈ 20s 转录）
            _save_sec = max(1, int(_sz / 2))
            # 文件扩展名判断是否音视频
            _is_media = uploaded.name.rsplit(".", 1)[-1].lower() in {
                "mp4","avi","mov","mkv","flv","wmv","mp3","m4a","wav","aac","ogg","flac"
            }
            if _is_media:
                # 音视频：额外预估转录时间（300MB mp4 ≈ 30min 视频 ≈ 60s 转录）
                _transcode_sec = max(10, int(_sz * 0.2))
                _total_sec = _save_sec + _transcode_sec
            else:
                _total_sec = _save_sec
            if _total_sec < 60:
                _eta_str = f"预计还需 {_total_sec} 秒"
            else:
                _eta_str = f"预计还需 {_total_sec // 60} 分 {_total_sec % 60} 秒"
            # 进度条：文件已完成上传到浏览器，保存到服务器 = 约 40%，后续处理 = 60%
            # 此处展示"就绪"状态（40% 进度），表示文件已就绪等待处理
            _bar_pct = 40
            # 文件图标
            _icon_map = {
...
<div class="upload-ready-card">
  <div class="upload-file-row">
    <span class="upload-file-icon">{_icon}</span>
    <span class="upload-file-name">{uploaded.name}</span>
    <span class="upload-file-size">{_sz_str}</span>
  </div>
  <div class="upload-progress-label">
    <span class="upload-progress-label-left">✅ 文件已就绪，等待处理</span>
    <span class="upload-progress-label-right">{_bar_pct}%</span>
  </div>
  <div class="upload-progress-track">
    <div class="upload-progress-fill" style="width:{_bar_pct}%;"></div>
  </div>
  <div class="upload-eta-row">
    点击「开始处理」后
    <span class="upload-eta-badge">⏱ {_eta_str}</span>
  </div>
</div>
""", unsafe_allow_html=True)
